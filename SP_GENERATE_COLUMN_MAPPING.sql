/*
================================================================================
STORED PROCEDURE: SP_GENERATE_COLUMN_MAPPING
================================================================================
Purpose: Generate column-level lineage mapping for Snowflake views
         Shows actual source table and column names (not aliases)

Usage:
  -- Single view
  CALL SP_GENERATE_COLUMN_MAPPING('SCHEMA.VIEW_NAME');
  
  -- Multiple views (comma-separated)
  CALL SP_GENERATE_COLUMN_MAPPING('SCHEMA.VIEW1, SCHEMA.VIEW2, SCHEMA.VIEW3');

Output: Returns a table with columns:
  - TARGET_TABLE, TARGET_COLUMN
  - SOURCE_TABLE, SOURCE_COLUMN  
  - TRANSFORMATION_LOGIC
  - CTE_CHAIN

Author: EDW Mapping Tool
Version: 4.0 (with nested subquery support)
================================================================================
*/

CREATE OR REPLACE PROCEDURE SP_GENERATE_COLUMN_MAPPING(VIEW_LIST VARCHAR)
RETURNS TABLE (
...
-- GRANT USAGE ON PROCEDURE SP_GENERATE_COLUMN_MAPPING(VARCHAR) TO ROLE YOUR_ROLE;
...
CALL SP_GENERATE_COLUMN_MAPPING('CORP_DESIGN_ISC.VW_BOM_STPO_STKO_BRP');

-- Example 2: Multiple views
CALL SP_GENERATE_COLUMN_MAPPING('STG.V_INVENTORY_FACT, STG.V_DIM_MATERIAL, STG.V_DIM_PLANT');

-- Example 3: With database prefix
CALL SP_GENERATE_COLUMN_MAPPING('EDW_DEV.CORP_DESIGN_ISC.VW_BOM_STPO_STKO_BRP');

-- Example 4: Save results to a table
CREATE OR REPLACE TABLE MY_SCHEMA.COLUMN_MAPPINGS AS
SELECT * FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));

-- Or use INSERT
INSERT INTO MY_SCHEMA.COLUMN_MAPPINGS
SELECT * FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));
